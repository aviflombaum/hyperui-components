...
RSpec.describe(HyperuiComponent::PaginationComponent, type: :component) do
  let(:count) { 30 }
  let(:items) { 5 }
  let(:size) { [1, 2, 2, 1] }
  ...
  subject! { render_inline(HyperuiComponent::PaginationComponent.new(**kwargs)) }
  ...
  context "link labels" do
    ...
    subject! do
      render_inline(HyperuiComponent::PaginationComponent.new) do |pagination|
  ...
  describe "not rendering" do
    ...
    context "when there are no pages or next/prev links" do
      subject! do
        render_inline(HyperuiComponent::PaginationComponent.new)
      end
  ...
  describe "overriding the next and previous text when using Pagy" do
    context "when the next text is overridden" do
      ...
      subject! { render_inline(HyperuiComponent::PaginationComponent.new(next_text: next_text, pagy: pagy)) }
    ...
    context "when the previous text is overridden" do
      ...
      subject! { render_inline(HyperuiComponent::PaginationComponent.new(previous_text: previous_text, pagy: pagy)) }
...
RSpec.describe(HyperuiComponent::PaginationComponent::PreviousPage, type: :component) do
  let(:component_css_class) { 'govuk-pagination__prev' }

  let(:kwargs) { { href: "#", text: "1" } }

  subject! { render_inline(HyperuiComponent::PaginationComponent::PreviousPage.new(**kwargs)) }

  it_behaves_like 'a component that accepts custom classes'
  it_behaves_like 'a component that accepts custom HTML attributes'
end

RSpec.describe(HyperuiComponent::PaginationComponent::NextPage, type: :component) do
  let(:component_css_class) { 'govuk-pagination__next' }

  let(:kwargs) { { href: "#", text: "1" } }

  subject! { render_inline(HyperuiComponent::PaginationComponent::NextPage.new(**kwargs)) }

  it_behaves_like 'a component that accepts custom classes'
  it_behaves_like 'a component that accepts custom HTML attributes'
end

RSpec.describe(HyperuiComponent::PaginationComponent::Item, type: :component) do
  let(:component_css_class) { 'govuk-pagination__item' }

  let(:kwargs) { { href: "#", number: 2 } }

  subject! { render_inline(HyperuiComponent::PaginationComponent::Item.new(**kwargs)) }
