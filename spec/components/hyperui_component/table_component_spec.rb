...
RSpec.describe(HyperuiComponent::TableComponent, type: :component) do
  ...
  subject! do
    render_inline(HyperuiComponent::TableComponent.new(**kwargs)) do |table|
  ...
  describe "passing data directly into the table component" do
    describe "captions" do
      ...
      context "when the caption is passed in as an argument" do
        ...
        subject! do
          render_inline(HyperuiComponent::TableComponent.new(**kwargs.merge(rows: rows, caption: caption_text)))
        end
    ...
    context "when the rows are passed in as a multidimensional array" do
      context "of arrays" do
        context "when the first row contains headers and there are three rows of data" do
          ...
          subject! do
            render_inline(HyperuiComponent::TableComponent.new(**kwargs.merge(rows: rows)))
          end
        ...
        context "when there are three rows of data and the head and foot are passed in separately" do
          ...
          subject! do
            render_inline(HyperuiComponent::TableComponent.new(**kwargs.merge(head: head, rows: rows, foot: foot)))
          end
      ...
      context "of hashes with a numeric column" do
        ...
        subject! do
          render_inline(
            HyperuiComponent::TableComponent.new(
    ...
    context "when :first_cell_is_header is true" do
      ...
      subject! do
        render_inline(HyperuiComponent::TableComponent.new(**kwargs.merge(head: head, rows: rows, first_cell_is_header: true)))
      end
  ...
  context "when the rows are built using nested slots" do
    subject! do
      render_inline(HyperuiComponent::TableComponent.new) do |table|
        table.head do |head|
          head.row do |row|
  ...
  describe "captions" do
    ...
    describe "when an invalid size is supplied" do
      ...
      let(:caption) { HyperuiComponent::TableComponent::CaptionComponent.new(text: "Bad size", size: invalid_size) }
  ...
  describe "custom colum widths" do
    subject! do
      render_inline(HyperuiComponent::TableComponent.new) do |table|
        table.head do |head|
          head.row do |row|
            HyperuiComponent::TableComponent::CellComponent::WIDTHS.each_key do |width|
    ...
    specify "adds the width class correctly" do
      HyperuiComponent::TableComponent::CellComponent::WIDTHS.each_key do |width, expected_class|
...
RSpec.describe(HyperuiComponent::TableComponent::HeadComponent, type: :component) do
...
RSpec.describe(HyperuiComponent::TableComponent::BodyComponent, type: :component) do
...
RSpec.describe(HyperuiComponent::TableComponent::RowComponent, type: :component) do
...
RSpec.describe(HyperuiComponent::TableComponent::CellComponent, type: :component) do
  ...
  describe "controlling scopes" do
    subject! do
      render_inline(HyperuiComponent::TableComponent::RowComponent.new(parent: 'tbody')) do |row|
        row.cell(
          text: "ABC",
          scope: false,
          header: true,
...
RSpec.describe(HyperuiComponent::TableComponent::CaptionComponent, type: :component) do
...
RSpec.describe(HyperuiComponent::TableComponent::FootComponent, type: :component) do
