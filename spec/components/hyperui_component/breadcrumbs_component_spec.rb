...
RSpec.describe(HyperuiComponent::BreadcrumbsComponent, type: :component) do
  let(:breadcrumbs) do
    {
      "Home"                 => "/level-one",
      "Products"             => "/level-two",
  ...
  subject! { render_inline(HyperuiComponent::BreadcrumbsComponent.new(**kwargs)) }
  ...
  context 'when an invalid breadcrumbs object is provided' do
    specify "raises an error" do
      expect { HyperuiComponent::BreadcrumbsComponent.new(breadcrumbs: "invalid") }.to raise_error(ArgumentError, "breadcrumbs must be an array or hash")
    end
  end
