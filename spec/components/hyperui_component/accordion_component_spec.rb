...
RSpec.describe(HyperuiComponent::AccordionComponent, type: :component) do
  ...
  subject! do
    render_inline(HyperuiComponent::AccordionComponent.new(**kwargs)) do |component|
      helper.safe_join(
  ...
  context 'when a custom ID is provided' do
    before do
      render_inline(HyperuiComponent::AccordionComponent.new(**kwargs))
    end
  ...
  describe 'overriding the section heading level' do
    ...
    context 'when the heading level is invalid' do
      specify 'has the overriden level' do
        ...
        expect { HyperuiComponent::AccordionComponent.new(heading_level: 8) }.to raise_error(ArgumentError, expected_message)
      end
    end
  end
  ...
  describe 'summaries' do
    ...
    context 'when a summary text is provided' do
      ...
      subject! do
        render_inline(HyperuiComponent::AccordionComponent.new) do |component|
