...
RSpec.describe(HyperuiComponent::WarningTextComponent, type: :component) do
  ...
  context 'when assistive text is overriden' do
    subject! { render_inline(HyperuiComponent::WarningTextComponent.new(**kwargs.merge(icon_fallback_text: custom_icon_fallback_text))) }
  ...
  context 'overriding the text with a custom HTML block' do
    ...
    subject! do
      render_inline(HyperuiComponent::WarningTextComponent.new(**kwargs)) { custom_html }
    end
  ...
  context "when the icon is overridden" do
    let(:custom_icon) { "?" }
    subject! { render_inline(HyperuiComponent::WarningTextComponent.new(**kwargs.merge(icon: custom_icon))) }
